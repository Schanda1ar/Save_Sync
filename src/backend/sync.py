...
def build_directory_manifest(path: Path) -> dict[str, str]:
    if not path.exists():
        return {}
    if not path.is_dir():
        raise SyncError(f"Save-Pfad ist kein Ordner: {path}")
...
class SaveSyncService:
    ...
    def run_profile(self, profile: GameProfile, status: StatusCallback | None = None) -> None:
        ...
        meta_path = self._meta_path(save_path)

        if not game_exe.exists():
            raise SyncError(f"Spiel-Executable nicht gefunden: {game_exe}")
        if save_path.exists() and not (save_path.is_file() or save_path.is_dir()):
            raise SyncError(f"Save-Pfad ist ungültig: {save_path}")
        ...
        report("Upload läuft")
        final_hash = snapshot_path(save_path)
    ...
    def _meta_path(self, save_path: Path) -> Path:
        if save_path.suffix:
            return save_path.with_suffix(".meta.json")
        return save_path.parent / f"{save_path.name}.meta.json"
    ...
    def _download_if_needed(self, drive, profile: GameProfile, save_path: Path):
        ...
        local_hash = snapshot_path(save_path)
        cloud_hash = None

        if remote_file is not None:
            if self._is_directory_target(save_path, remote_file):
        ...
        if cloud_hash and cloud_hash != local_hash:
            self._backup_existing(save_path)
            if self._is_directory_target(save_path, remote_file):
                self._download_directory(remote_file, save_path)
            else:
    ...
    def _upload_if_needed(
        ...
        if final_hash is None:
            raise SyncError(f"Save-Pfad nicht gefunden: {save_path}")
        ...
        if save_path.is_dir():
            self._upload_directory(drive_file, save_path)
            save_meta({"hash": final_hash, "kind": "directory"}, meta_path)
            return

        drive_file["description"] = "savesync:file"
        drive_file.SetContentFile(str(save_path))
        drive_file.Upload()
        save_meta({"hash": final_hash, "kind": "file"}, meta_path)
    ...
    def _is_directory_target(self, save_path: Path, remote_file) -> bool:
        if save_path.exists():
            return save_path.is_dir()
        if remote_file is None:
            return False
        return remote_file.get("description") == "savesync:directory"
    ...
    def _download_directory(self, remote_file, save_path: Path) -> None:
        with tempfile.TemporaryDirectory() as temp_dir_name:
            temp_dir = Path(temp_dir_name)
            archive_path = temp_dir / "save_archive.zip"
            extracted_path = temp_dir / "extracted"
            ...
            with zipfile.ZipFile(archive_path, "r") as archive:
                archive.extractall(extracted_path)
            self._replace_directory(save_path, extracted_path)

    def _upload_directory(self, drive_file, save_path: Path) -> None:
        with tempfile.NamedTemporaryFile(delete=False, suffix=".zip") as temp_file:
            archive_path = Path(temp_file.name)
        try:
            with zipfile.ZipFile(archive_path, "w", compression=zipfile.ZIP_DEFLATED) as archive:
                for file_path in sorted(item for item in save_path.rglob("*") if item.is_file()):
                    archive.write(file_path, arcname=file_path.relative_to(save_path).as_posix())
    ...
    def _backup_existing(self, save_path: Path) -> None:
        if not save_path.exists():
            return
        timestamp = int(time.time())
        if save_path.is_dir():
            backup_path = save_path.parent / f"{save_path.name}_backup_{timestamp}"
            shutil.copytree(save_path, backup_path)
            return
        backup_path = save_path.with_name(f"{save_path.stem}_backup_{timestamp}{save_path.suffix}")
